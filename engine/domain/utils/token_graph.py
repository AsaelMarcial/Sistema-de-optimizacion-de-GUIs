...
def _bounds_intersect(left: Element, right: Element) -> bool:
    return not (
...
def _normalize_id_map(payload: Mapping[str, Any] | None) -> dict[str, tuple[str, ...]]:
    if not payload:
        return {}
...
def _element_index(elements: Sequence[Element]) -> dict[str, Element]:
    return {entry.node_id: entry for entry in elements}


def _style_index(styles: StyleInventoryModel) -> dict[str, StyleInventoryEntry]:
    return {entry.style_id: entry for entry in styles}


def _color_index(colors: ColorInventoryModel) -> dict[str, ColorInventoryEntry]:
    return {entry.color_id: entry for entry in colors}


def _palette_index(palettes: Sequence[TonalPaletteModel]) -> dict[str, TonalPaletteModel]:
    return {palette.palette_id: palette for palette in palettes}


def _token_index(tokens: TokenInventoryModel) -> dict[str, TokenModel]:
    return {entry.token_id: entry for entry in tokens if entry.token_id}


def _visible_elements(elements: Sequence[Element]) -> tuple[Element, ...]:
    return tuple(entry for entry in elements if entry.is_visible)
        ...
        if token.tone is not None:
            for palette_id in token.source_palette_ids:
                ...
                if normalized_palette_id:
                    palette_tone_to_token_ids[f"{normalized_palette_id}:{int(token.tone)}"].append(token_id)
...
def _build_adjacency(elements: Sequence[Element]) -> dict[str, tuple[str, ...]]:
    element_lookup = _element_index(elements)
    visible_entries = _visible_elements(elements)
    ...
    for entry in elements:
        ...
        if entry.parent_id:
            ...
            if parent is not None:
                for sibling_id in parent.children_ids:
                    ...
                    if sibling is not None and sibling.is_visible:
        ...
        if candidates:
            adjacency[entry.node_id] = tuple(
                item.node_id
                for item in sorted(candidates.values(), key=lambda item: (item.document_order, item.node_id))
            )
    return adjacency
...
class TokenGraph:
    ...
    _elements_ref: tuple[Element, ...] = field(default_factory=tuple, repr=False, compare=False)
    _prototype_structure_ref: PrototypeStructure | None = field(default=None, repr=False, compare=False)
    _styles_ref: StyleInventoryModel = field(default_factory=StyleInventoryModel, repr=False, compare=False)
    _colors_ref: ColorInventoryModel = field(default_factory=ColorInventoryModel, repr=False, compare=False)
    _tokens_ref: TokenInventoryModel = field(default_factory=TokenInventoryModel, repr=False, compare=False)
    _element_ref_index: Mapping[str, Element] = field(default_factory=dict, repr=False, compare=False)
    _style_ref_index: Mapping[str, StyleInventoryEntry] = field(default_factory=dict, repr=False, compare=False)
    _color_ref_index: Mapping[str, ColorInventoryEntry] = field(default_factory=dict, repr=False, compare=False)
    _palette_ref_index: Mapping[str, TonalPaletteModel] = field(default_factory=dict, repr=False, compare=False)
    _token_ref_index: Mapping[str, TokenModel] = field(default_factory=dict, repr=False, compare=False)
    ...
    def build(
        ...
        *,
        ...
        elements = tuple(prototype_structure.nodes)
        element_to_style_ids = {
            entry.node_id: _unique(
                property_model.style_id or ""
        ...
        return cls(
            ...
            token_ids=token_ids,
            root_ids=tuple(entry.node_id for entry in elements if entry.parent_id is None),
            ...
            _prototype_structure_ref=prototype_structure,
            _styles_ref=styles,
            _colors_ref=colors,
    ...
    def build_from_artifact(
        ...
        *,
        prototype_structure: PrototypeStructure | None = None,
        ...
        graph = cls(
            element_ids=_unique(payload.get("element_ids") or ()),
            style_ids=_unique(
        ...
        if prototype_structure is None and styles is None and colors is None and palettes is None and tokens is None:
            return graph
    ...
    @property
    def prototype_structure(self) -> PrototypeStructure | None:
        return self._prototype_structure_ref
    ...
    def bind_inventories(
        self,
        *,
        prototype_structure: PrototypeStructure | None,
        styles: StyleInventoryModel,
        colors: ColorInventoryModel,
        palettes: Sequence[TonalPaletteModel] | None = None,
        tokens: TokenInventoryModel | None = None,
        ...
        elements = tuple(prototype_structure.nodes) if prototype_structure is not None else self.elements
        return replace(
            self,
            token_ids=token_ids,
            palettes=palette_entries,
    ...
    def element_by_id(self, node_id: str) -> Element | None:
    ...
    def tokens_for_element(self, node_id: str) -> tuple[TokenModel, ...]:
        ...
        return tuple(token for token in (self.token_by_id(token_id) for token_id in token_ids) if token is not None)
    ...
    def visible_elements(self) -> tuple[Element, ...]:
        return _visible_elements(self.elements)

    def parent_of(self, node_id: str) -> Element | None:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.parent_of(node_id)
    ...
    def children_of(self, node_id: str) -> tuple[Element, ...]:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.children_of(node_id)
    ...
    def ancestors_of(self, node_id: str) -> tuple[Element, ...]:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.ancestors_of(node_id)
    ...
    def descendants_of(self, node_id: str) -> tuple[Element, ...]:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.descendants_of(node_id)
    ...
    def siblings_of(self, node_id: str) -> tuple[Element, ...]:
        entry = self.element_by_id(node_id)
        if entry is None or entry.parent_id is None:
            return ()
        return tuple(item for item in self.children_of(entry.parent_id) if item.node_id != node_id)

    def previous_visible_sibling(self, node_id: str) -> Element | None:
        siblings = sorted(
            [item for item in self.siblings_of(node_id) if item.is_visible],
    ...
    def next_visible_sibling(self, node_id: str) -> Element | None:
        siblings = sorted(
            [item for item in self.siblings_of(node_id) if item.is_visible],
            key=lambda item: item.document_order,
        )
    ...
    def effective_background_of(self, node_id: str) -> ColorInventoryEntry | None:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.effective_background_of(node_id, self.colors)
        entry = self.element_by_id(node_id)
        if entry is None:
            return None
        if entry.effective_background:
            background = self.colors.entry_by_value(entry.effective_background)
        ...
        for candidate in (entry, *self.ancestors_of(node_id)):
            for property_model in candidate.properties:
                if property_model.classification != "background" or not property_model.color_id:
                    continue
                color_entry = self.color_by_id(property_model.color_id)
    ...
    def effective_color_of(self, node_id: str) -> ColorInventoryEntry | None:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.effective_color_of(node_id, self.colors)
        entry = self.element_by_id(node_id)
        if entry is None:
            return None
        for candidate in (entry, *self.ancestors_of(node_id)):
    ...
    def surface_container_of(self, node_id: str) -> Element | None:
        if self._prototype_structure_ref is not None:
            return self._prototype_structure_ref.surface_container_of(node_id)
        entry = self.element_by_id(node_id)
        if entry is None:
            return None
        for candidate in (entry, *self.ancestors_of(node_id)):
    ...
    def adjacent_elements_of(self, node_id: str) -> tuple[Element, ...]:
        ...
        if adjacent_ids is None:
            ...
            for other in self.visible_elements():
