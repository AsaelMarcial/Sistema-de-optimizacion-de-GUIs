...
class ContrastColorReferenceModel:
    css: str
    hex_value: str
    alpha: float
    color_id: str | None = None
    element_id: str | None = None
    color_property_id: str | None = None
    ...
    def build(cls, payload: Mapping[str, Any] | None = None) -> Self:
        ...
        return cls(
            ...
            color_property_id=(
                str(payload["color_property_id"])
                if payload.get("color_property_id") is not None
                else None
            ),
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.color_property_id is not None:
            payload["color_property_id"] = self.color_property_id
...
class ContrastIssueModel:
    ...
    foreground: ContrastColorReferenceModel = field(default_factory=lambda: ContrastColorReferenceModel("", "", 0.0))
    background: ContrastColorReferenceModel = field(default_factory=lambda: ContrastColorReferenceModel("", "", 0.0))
    ...
    def build(cls, payload: Mapping[str, Any] | None = None) -> Self:
        ...
        return cls(
            ...
            foreground=ContrastColorReferenceModel.build(
                payload.get("foreground") if isinstance(payload.get("foreground"), Mapping) else None
            ),
            background=ContrastColorReferenceModel.build(
...
@dataclass(frozen=True, slots=True)
class ContrastReportModel:
    issues: tuple[ContrastIssueModel, ...] = field(default_factory=tuple)
